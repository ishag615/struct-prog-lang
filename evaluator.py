...
def set_watch_context(watch_ctx):
    """Called by runner to inject watch context"""
    global _watch_context
    _watch_context = watch_ctx

def format_value_for_watch(val):
    """Format a value for watch display"""
    if isinstance(val, bool):
        ...
        items = [format_value_for_watch(v) for v in val]
        return f"[{', '.join(items)}]"
    elif isinstance(val, dict):
        items = [f'"{k}": {format_value_for_watch(v)}' for k, v in val.items()]
        return f"{{{', '.join(items)}}}"
    else:
        return str(val)
      
def notify_variable_change(var_name, new_value, source_info=""):
    """
    Notify watch system of variable changes
    This is called from within evaluate() when variables are assigned
    """
    global _watch_context
    if _watch_context and _watch_context.watch_enabled:
        _watch_context.record_change(var_name, new_value, source_info)
...
##############################################################################################################
  ...
  ################################ADDED SWITCH EVALUATE####################################
    ...
    if ast["tag"] == "assign":
        ...
        # NEW: Notify watch system of the change
        if target["tag"] == "identifier":
            notify_variable_change(target_index, value)
